...
public class LinkedList<T extends Object> extends List<T>{
	...
	public Object[] find(T value) {
		Object[] result=new Object[getTimes(value)];
		int times = 0;
		UniNode<T> temp = root;
		while (temp.next != null) {
			if (temp.next.value.equals(value)) {
				result[times]=temp.next;
				times++;
			}
			temp = temp.next;
			// last element of the list can cause a null pointer exception
			// here.So make a decide
			if (temp == null) {
				break;
			}
		}
...
//		System.out.println(test.toString());
		...
		test.append(c);
		System.out.println(test.toString());
		System.out.println(test.find("second").length);
	}
}
